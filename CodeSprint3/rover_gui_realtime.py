...
Le PC calcule la position via vitesse + temps (aucune modif du Pico).

Paramètres à calibrer si la carte est inexacte :
  CM_PER_SEC  : vitesse linéaire réelle à speed=50
  DEG_PER_SEC : vitesse angulaire réelle à speed=50
"""
...
# Rover speed=50 sur 1000 max, roue ⌀6cm
# RPM ≈ 50/1000 * 300 = 15 RPM  → 15 * π * 6 / 60 ≈ 4.7 cm/s
CM_PER_SEC  = 4.7    # cm/s en avance/recul  (à calibrer sur le vrai rover)
DEG_PER_SEC = 45.0   # °/s en rotation       (à calibrer sur le vrai rover)
...
class MapCanvas(tk.Canvas):
    """
    Affiche la trajectoire du rover et sa position courante.
    Origine (0,0) = point de départ.
    X = axe avant du rover au départ (vers le haut).
    Y = axe latéral (vers la droite).
    """
    ...
    def __init__(self, master, **kwargs):
        ...
        self.trail   = [(0.0, 0.0)]   # historique positions (x_cm, y_cm)
        self.rover_x = 0.0
        self.rover_y = 0.0
        self.rover_heading = 0.0      # degrés, 0 = axe +X
        self.bind("<Configure>", lambda e: self._redraw())

    # ── Coordonnées monde → canvas ─────────────────────────────────────────────
    def _to_px(self, x_cm, y_cm):
        w = self.winfo_width()
        h = self.winfo_height()
        cx = w // 2
        cy = h // 2
        # x_cm vers le haut → py diminue
        # y_cm vers la droite → px augmente
    ...
    def _redraw(self):
        ...
        for i in range(-10, 11):
            ...
            if 0 < py < h:
                self.create_text(cx + 4, py, anchor="w",
                                 text=f"{val}", fill=FG_DIM, font=("Courier New", 6))
            px2, py2 = self._to_px(0, val)
            if 0 < px2 < w:
                self.create_text(px2, cy - 4, anchor="s",
                                 text=f"{val}", fill=FG_DIM, font=("Courier New", 6))
        ...
        if len(self.trail) >= 2:
            ...
            for (x, y) in self.trail:
                ...
                pts.extend([px, py])
            self.create_line(*pts, fill=TRAIL_COLOR, width=2,
    ...
    def _draw_rover(self):
        ...
        s  = 10
        # heading 0° = avance selon +X (vers le haut du canvas)
        # sur le canvas : angle 0 = haut → -π/2 en coordonnées standard canvas
        ...
        self.create_polygon(tip[0], tip[1], bl[0], bl[1], br[0], br[1],
                            fill=ROVER_COLOR, outline="#ff7070", width=1)
    ...
    def update_pos(self, x, y, heading):
        ...
        # Ajoute à la trajectoire si déplacement suffisant
...
class Odometry:
    """
    Calcule la position (x, y, heading) du rover côté PC.
    Appelée à chaque tick de 50 ms.
    """

    def __init__(self):
        self.x       = 0.0
        self.y       = 0.0
        self.heading = 0.0   # degrés
        self.cmd     = None 
        self.WHEEL_DIAMETER = 6  # cm
        self.MAX_RPM = 300
         # commande active : 'z','s','q','d','x' ou None
    ...
    def tick(self, dt: float):
        ...
        speed = 50  # même vitesse que ton rover
        ...
        elif self.cmd == "q":  # rotation gauche
        # ⚠️ dépend de l'écartement des roues
            WHEEL_BASE = 12  # distance entre roues gauche/droite (cm)
            angular_speed = (v / WHEEL_BASE) * (180 / math.pi)
            self.heading -= angular_speed * dt

        elif self.cmd == "d":  # rotation droite
            WHEEL_BASE = 12
            angular_speed = (v / WHEEL_BASE) * (180 / math.pi)
            self.heading += angular_speed * dt

        elif self.cmd == "x":  # strap
...
class RoverControl:
    ...
    TICK_MS = 50   # refresh carte toutes les 50 ms
    ...
    def __init__(self, root: tk.Tk):
        ...
        self._build()
        self._bind_keys()
        self._tick()   # démarrage de la boucle d'odométrie
    ...
    def _build_controls(self, parent):
        # Titre
        tk.Label(parent, text="◈ ROVER CONTROL",
                 font=FONT_XL, fg=ACCENT, bg=BG,
                 pady=14).pack()
        ...
        tk.Label(cf, text="IP", font=FONT_SM, fg=FG_DIM, bg=BG,
                 width=3).pack(side="left")
        tk.Entry(cf, textvariable=self._ip_var, font=FONT_SM,
                 bg=PANEL, fg=FG, insertbackground=FG,
                 relief="flat", bd=4, width=16).pack(side="left", padx=(4, 10))
        self._status_dot = tk.Label(cf, text="●", font=("Courier New", 14),
                                    fg=RED, bg=BG)
        self._status_dot.pack(side="right", padx=(0, 4))
        self._conn_btn = tk.Button(cf, text="CONNECT", font=FONT,
                                   bg=GREEN, fg="black",
        ...
        self._make_btn(r2, "◀\nGAUCHE", "q", BTN_BG, w=10, h=3).pack(side="left", padx=2, pady=2)
        self._make_btn(r2, "■\nSTOP",   "n", RED,    w=10, h=3).pack(side="left", padx=2, pady=2)
        self._make_btn(r2, "▶\nDROITE", "d", BTN_BG, w=10, h=3).pack(side="left", padx=2, pady=2)

        r3 = tk.Frame(pad, bg=BG); r3.pack()
        self._make_btn(r3, "▼\nRECULER", "s", BTN_BG, w=14, h=3).pack(pady=2)

        r4 = tk.Frame(pad, bg=BG); r4.pack(pady=(8, 0))
        self._make_btn(r4, "◀▶  STRAP LATÉRAL", "x", YELLOW,
                       w=34, h=2, fg_color="black").pack()

        tk.Frame(parent, bg=BORDER, height=1).pack(fill="x", padx=20, pady=10)

        # Hints
        tk.Label(parent,
                 text="Z/↑ Avancer  S/↓ Reculer  Q/← Gauche\n"
                      "D/→ Droite   X Strap   N/Espace Stop",
                 font=FONT_SM, fg=FG_DIM, bg=BG, justify="left").pack(padx=20)
        ...
        self._log_txt = tk.Text(lf, height=5, bg=PANEL, fg=FG_DIM,
    ...
    def _build_map(self, parent):
        ...
        tk.Button(header, text="RESET", font=FONT_SM,
                  bg=BTN_BG, fg=FG_DIM,
                  activebackground=BTN_BG, relief="flat",
                  cursor="hand2", padx=6,
                  command=self._reset_map).pack(side="right")
        tk.Label(header, text="▲=Nord   grille=25cm",
                 font=FONT_SM, fg=FG_DIM, bg=BG).pack(side="right", padx=10)
    ...
    def _make_btn(self, parent, label, cmd, color, w=10, h=2, fg_color="white"):
        b = tk.Button(parent, text=label, font=FONT, width=w, height=h,
    ...
    def _connect(self):
        ...
        try:
            ...
            self.sock      = s
            self.connected = True
            self._conn_btn.config(text="DISCONNECT", bg=RED, fg="white",
            ...
            messagebox.showerror("Connexion impossible",
                                 f"Impossible de joindre {ip}:{PORT}\n\n{e}")
    ...
    def _disconnect(self):
        ...
        if self.sock:
            ...
            self.sock = None
        self.odo.cmd = None
        self._conn_btn.config(text="CONNECT", bg=GREEN, fg="black",
                              activebackground=GREEN)
        self._status_dot.config(fg=RED)
        self._write_log("Déconnecté.")

    # ─────────────────────────────────────────────────────────────────────────
    #  ENVOI COMMANDES + MAJ ODOMÉTRIE
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _send(self, cmd: str):
        ...
        # Mise à jour odométrie locale (indépendante de la connexion)
        if cmd in ("z", "s", "q", "d", "x"):
            self.odo.cmd = cmd
        elif cmd in ("n", "exit"):
            self.odo.cmd = None
